...
{
    private int _customResultIndex;
    private static bool _rememberPreference;
    ...
    /// <summary>
    /// Constructor used when custom button labels are provided.
    /// <paramref name="buttonLabels"/> determines the button count and text.
    /// </summary>
    private CustomMessageBox(string message, string title, string[] buttonLabels)
    {
        InitializeComponent();
        ...
        SetCustomButtons(buttonLabels);

        PreviewKeyDown += OnPreviewKeyDown;
    }

    private void SetCustomButtons(string[] labels)
    {
        ...
        for (var i = 0; i < labels.Length; i++)
        {
            ...
            {
                Content = labels[i],
                Style = (Style)(isLast ? FindResource("MessageButton") : FindResource("SecondaryButton")),
            ...
            button.Click += (_, _) =>
            {
                _customResultIndex = index;
                DialogResult = true;
                Close();
            };

            if (isLast) button.IsDefault = true;
    ...
    /// <summary>
    /// Shows a message box with custom button labels. Returns the index of
    /// the button the user clicked (0-based).
    /// </summary>
    public static int Show(string message, string title, params string[] buttons)
    {
        ...
        return messageBox._customResultIndex;
    }

    /// <summary>
    /// Shows a message box with custom button labels and a "Remember my
    /// preference and don't ask me again" checkbox. Returns the index of
    /// the button clicked and whether the checkbox was checked.
    /// </summary>
    public static (int Index, bool Remember) ShowWithRemember(
        string message, string title, params string[] buttons)
    {
        var messageBox = new CustomMessageBox(message, title, buttons);
        SetOwnerWindow(messageBox);
        messageBox.ChkRemember.Visibility = Visibility.Visible;
        messageBox.ChkRemember.IsChecked = _rememberPreference;
        messageBox.ShowDialog();
        _rememberPreference = messageBox.ChkRemember.IsChecked == true;
        return (messageBox._customResultIndex, _rememberPreference);
    }
    ...
    private void AddButton(string text, MessageBoxResult result, bool isPrimary)
    {
        var button = new Button
        {
            Content = text,
            Style = isPrimary
                ? (Style)FindResource("MessageButton")
    ...
    /// <summary>
    /// Shows a message box with rich formatted content using inline elements
    /// (Run, Bold, LineBreak, etc.) instead of plain text.
    /// </summary>
    public static void ShowFormatted(
        string title,
        IEnumerable<Inline> inlines,
        MessageBoxImage icon = MessageBoxImage.None)
    {
        var messageBox = new CustomMessageBox(string.Empty, title, MessageBoxButton.OK, icon);
        messageBox.MessageText.Inlines.Clear();

        foreach (var inline in inlines)
            messageBox.MessageText.Inlines.Add(inline);
